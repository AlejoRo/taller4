...
public class Servlet1 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            //Para crear la sesion, y si ya existe la traemos
            HttpSession sesion = request.getSession(true);

            //Buscamos si la sesion ya tiene el objeto banco
            Banco banco = (Banco) sesion.getAttribute("banco");

            //Validar la primera vez que se crea la sesion
            if (banco == null) {
                banco = new Banco();
                banco.nuevaCuenta(1, true);
                banco.nuevaCuenta(2, true);
            ...
            if (formulario.equals("accion1")) {
                ...
                boolean extranjero = !(nacionalidad.equals("local"));
